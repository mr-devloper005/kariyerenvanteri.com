...
const inputClass = 'h-12 w-full rounded-full border border-[#0d0d0d1a] bg-[#f2f0ed] px-4 text-sm font-medium text-[#0d0d0d] outline-none transition placeholder:text-[#0d0d0d99] focus:border-[#cc1c4b]'
const buttonClass = 'inline-flex h-12 w-full items-center justify-center rounded-full bg-[#cc1c4b] px-6 text-sm font-medium uppercase tracking-[0.14em] text-white transition hover:bg-[#0d0d0d] disabled:cursor-not-allowed disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1rem] px-4 py-3 text-sm font-medium ${status === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-[#c6a6ff] text-[#0d0d0d]'}`}>{message}</p> : null}
